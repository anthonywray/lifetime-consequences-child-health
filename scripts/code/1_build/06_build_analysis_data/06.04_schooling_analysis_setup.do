...
************
* SCRIPT: 06.04_schooling_analysis_setup.do
* PURPOSE: This do file merges all HOSP-to-I-CeM and I-CeM-to-I-CeM linked data for the analysis of schooling outcomes
************

*********************************************************************************************************************
*********** ICEM HOSP Scholar Merge Matched Files *************
*********************************************************************************************************************

/* Extract census records matched to hospital patients from ICEM-ICEM linked files
	NOTE: 1901 census did not record scholar as occupation for children */
	
forvalues y = 1881(10)1891 {
	...
	save "$PROJ_PATH/processed/temp/scholar_hosp_icem_matches_`y'.dta", replace
}
clear
forvalues y = 1881(10)1891 {
	append using "$PROJ_PATH/processed/temp/scholar_hosp_icem_matches_`y'.dta"
}
sort censusyr RecID sex
save "$PROJ_PATH/processed/temp/scholar_hosp_icem_matches.dta", replace
...
merge m:1 censusyr RecID sex using "$PROJ_PATH/processed/temp/scholar_hosp_icem_matches.dta", assert(1 3) keep(3) nogen
...
save "$PROJ_PATH/processed/intermediate/crosswalks/scholar_hosp_icem_links_combined.dta", replace



// Resolve matches to multiple censuses 
use "$PROJ_PATH/processed/intermediate/crosswalks/scholar_hosp_icem_links_combined.dta", clear
...
foreach patid in regid pat_id_proxy {
	...
	* Prioritize match to census in age range 5-12
	gen school_age_match = (age_ICEM >= 5 & age_ICEM <= 12)
	egen tot_sam = total(school_age_match == 1), by(`patid')
	drop if tot_sam > 0 & school_age_match == 0
	drop tot_sam school_age_match
...
sort pat_id_proxy patient_id regid censusyr RecID
save "$PROJ_PATH/processed/temp/scholar_mmf_patient_id_reshape.dta", replace
...
save "$PROJ_PATH/processed/temp/scholar_mmf_regids.dta", replace

// Save crosswalk
use pat_id_proxy censusyr RecID using "$PROJ_PATH/processed/temp/scholar_mmf_patient_id_reshape.dta", clear
bysort pat_id_proxy censusyr RecID: keep if _n == 1
joinby pat_id_proxy using "$PROJ_PATH/processed/temp/scholar_mmf_regids.dta"
drop pat_id_proxy
duplicates drop
drop if regid == .
unique regid
sort censusyr RecID
save "$PROJ_PATH/processed/temp/scholar_hosp_icem_crosswalk_inprog.dta", replace
...
use "$PROJ_PATH/processed/temp/scholar_mmf_patient_id_reshape.dta", clear
drop pat_id_proxy patient_id regid admityr
duplicates drop
joinby censusyr RecID using "$PROJ_PATH/processed/temp/scholar_hosp_icem_crosswalk_inprog.dta", unmatched(both)
tab _merge
drop _merge
...
* Prioritize match to census in age range 5-12
gen school_age_match = (age_ICEM >= 5 & age_ICEM <= 12)
egen tot_sam = total(school_age_match == 1), by(regid)
drop if tot_sam > 0 & school_age_match == 0
drop tot_sam school_age_match
...
save "$PROJ_PATH/processed/intermediate/crosswalks/scholar_hosp_icem_crosswalk.dta", replace
drop regid
duplicates drop
unique censusyr RecID
save "$PROJ_PATH/processed/temp/scholar_hosp_icem_matched_ids.dta", replace

// Remove files

forvalues y = 1881(10)1891 {
	rm "$PROJ_PATH/processed/temp/scholar_hosp_icem_matches_`y'.dta"
}
rm "$PROJ_PATH/processed/temp/scholar_hosp_icem_matches.dta"
rm "$PROJ_PATH/processed/temp/scholar_mmf_regids.dta"
rm "$PROJ_PATH/processed/temp/scholar_mmf_patient_id_reshape.dta"
rm "$PROJ_PATH/processed/temp/scholar_hosp_icem_crosswalk_inprog.dta"



*********************************************************************************************************************
*********** ICEM HOSP Scholar Analysis Input *************
*********************************************************************************************************************
...
forvalues t_1 = 1881(10)1891 {
	...
	* Merge with variables needed for analysis
	...
	merge 1:1 censusyr RecID using "$PROJ_PATH/processed/temp/scholar_hosp_icem_matched_ids.dta", keep(1 3) keepusing(censusyr RecID yr_1st_admit)
	tab patient _merge
	...
	*	(a) You are in infancy (age 0-2)
	...
	merge 1:1 Year RecID using "$PROJ_PATH/processed/intermediate/icem/self_vars/icem_self_`t_1'_demographic.dta", assert(2 3) keep(3) nogen keepusing(Rela Age)
	...
	merge 1:1 Year RecID using "$PROJ_PATH/processed/intermediate/icem/self_vars/icem_self_`t_1'_occupation.dta", assert(2 3) keep(3) nogen keepusing(Occ Occode)
	...
	***** Changes to ICEM county names:
	...
	tempfile ICEM_Scholar_`t_1'
	save `ICEM_Scholar_`t_1'', replace
}

clear
forvalues t_1 = 1881(10)1891 {
	append using `ICEM_Scholar_`t_1''
}
duplicates drop

save "$PROJ_PATH/processed/temp/scholar_hosp_icem_analysis_input.dta", replace

*********************************************************************************************************************
*********** ICEM HOSP Analysis Setup *************
*********************************************************************************************************************

use "$PROJ_PATH/processed/temp/scholar_hosp_icem_analysis_input.dta", clear

* Restrict to final HOSP-ICEM-ICEM matched sample
merge m:1 censusyr RecID sex using "$PROJ_PATH/processed/temp/scholar_hosp_icem_matched_ids.dta", assert(1 3) keep(1 3) keepusing(censusyr RecID sex)
...
drop ic_occ ic_occode ic_relate
...
gen valid_age = (age_child >= 3 & age_child <= 15)
tab valid_age
...
merge 1:1 censusyr RecID using "$PROJ_PATH/processed/temp/scholar_hosp_icem_matched_ids.dta", keep(1 3) keepusing(age_dist mi_mismatch rescty_match distpar_match nodist_match jw_fname_orig-jw_name similar_0-similar_15) 
tab patient _merge
...
save "$PROJ_PATH/processed/temp/scholar_hosp_icem_analysis_variables.dta", replace

* Create patient specific variables

use censusyr RecID patient patient_id using "$PROJ_PATH/processed/temp/scholar_hosp_icem_analysis_variables.dta" if patient == 1, clear
drop patient
merge 1:m censusyr RecID using "$PROJ_PATH/processed/intermediate/crosswalks/scholar_hosp_icem_crosswalk.dta", assert(2 3) keep(3) nogen keepusing(regid)
...
save "$PROJ_PATH/processed/temp/scholar_hosp_analysis_variables.dta", replace

use "$PROJ_PATH/processed/temp/scholar_hosp_icem_analysis_variables.dta", clear
merge 1:1 patient_id using "$PROJ_PATH/processed/temp/scholar_hosp_analysis_variables.dta", assert(1 3)
tab patient _merge
drop _merge
...
sort censusyr hhid pid
save "$PROJ_PATH/processed/intermediate/final_build/schooling_analysis_setup.dta", replace

rm "$PROJ_PATH/processed/temp/scholar_hosp_icem_analysis_variables.dta"
rm "$PROJ_PATH/processed/temp/scholar_hosp_icem_matched_ids.dta"
rm "$PROJ_PATH/processed/temp/scholar_hosp_icem_analysis_input.dta"
rm "$PROJ_PATH/processed/temp/scholar_hosp_analysis_variables.dta"
