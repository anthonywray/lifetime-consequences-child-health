...
************
* SCRIPT: 06.05_pre_existing_analysis_setup.do
* PURPOSE: This do file merges all HOSP-to-I-CeM and I-CeM-to-I-CeM linked data for the analysis of pre-existing disability outcomes
************

*********************************************************************************************************************
*********** ICEM HOSP PreExisting Merge Matched Files *************
*********************************************************************************************************************
...
forvalues y = 1881(10)1901 {
	...
	save "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matches_`y'.dta", replace
}
clear
forvalues y = 1881(10)1901 {
	append using "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matches_`y'.dta"
}
...
merge m:1 censusyr RecID sex using "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matches_`y'.dta", assert(1 3) keep(3) nogen
...
save "$PROJ_PATH/processed/intermediate/crosswalks/pre_exist_hosp_icem_links_combined.dta", replace

***************** Resolve matches to multiple censuses *************************

use "$PROJ_PATH/processed/intermediate/crosswalks/pre_exist_hosp_icem_links_combined.dta", clear
...
sort pat_id_proxy patient_id regid censusyr RecID
save "$PROJ_PATH/processed/temp/pre_exist_mmf_patient_id_reshape.dta", replace
...
save "$PROJ_PATH/processed/temp/pre_exist_mmf_regids.dta", replace

* Save crosswalk
use pat_id_proxy censusyr RecID using "$PROJ_PATH/processed/temp/pre_exist_mmf_patient_id_reshape.dta", clear
bysort pat_id_proxy censusyr RecID: keep if _n == 1
joinby pat_id_proxy using "$PROJ_PATH/processed/temp/pre_exist_mmf_regids.dta"
drop pat_id_proxy
duplicates drop
drop if regid == .
unique regid
sort censusyr RecID
save "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_crosswalk_inprog.dta", replace
...
use "$PROJ_PATH/processed/temp/pre_exist_mmf_patient_id_reshape.dta", clear
drop pat_id_proxy patient_id regid admityr
duplicates drop
joinby censusyr RecID using "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_crosswalk_inprog.dta", unmatched(both)
tab _merge
drop _merge
...
* Drop census records linked to both boys and girls

egen tot_male = total(sex == 1), by(censusyr RecID)
egen tot_female = total(sex == 2), by(censusyr RecID)
drop if tot_male > 0 & tot_female > 0
drop tot_male tot_female
...
save "$PROJ_PATH/processed/intermediate/crosswalks/pre_exist_hosp_icem_crosswalk.dta", replace
drop regid
duplicates drop
unique censusyr RecID
save "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matched_ids.dta", replace

* Remove files

forvalues y = 1881(10)1901 {
	rm "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matches_`y'.dta"
}
rm "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matches_`y'.dta"

rm "$PROJ_PATH/processed/temp/pre_exist_mmf_regids.dta"
rm "$PROJ_PATH/processed/temp/pre_exist_mmf_patient_id_reshape.dta"
rm "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_crosswalk_inprog.dta"

*********************************************************************************************************************
*********** ICEM HOSP PreExisting Analysis Input *************
*********************************************************************************************************************
...
forvalues t_1 = 1881(10)1901 {
	...
	* Merge with variables needed for analysis
	...
	merge 1:1 censusyr RecID using "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matched_ids.dta", keep(1 3) keepusing(censusyr RecID yr_1st_admit)
	tab patient _merge
	...
	qui {
		...
		*	(b) You are the same age as the age of the youngest patient in the year of the first admission
		...
		forvalues y = `min_year'(1)`max_year' {
			...
			qui replace hh_0to5_hosp = `tot_0to5_in_`y'' if `y' == `ref_yr'
			drop `tot_0to5_in_`y''
		}
	...
	***** Changes to ICEM county names:
	...
	tempfile ICEM_PreExisting_`t_1'
	save `ICEM_PreExisting_`t_1'', replace
}

clear
forvalues t_1 = 1881(10)1901 {
	append using `ICEM_PreExisting_`t_1''
}
duplicates drop

save "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_analysis_input.dta", replace

*********************************************************************************************************************
*********** ICEM HOSP Analysis Setup *************
*********************************************************************************************************************

use "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_analysis_input.dta", clear

* Restrict to final HOSP-ICEM-ICEM matched sample
merge m:1 censusyr RecID sex using "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matched_ids.dta", assert(1 3) keep(1 3) keepusing(censusyr RecID sex)
...
merge 1:1 censusyr RecID using "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matched_ids.dta", keep(1 3) keepusing(age_dist mi_mismatch rescty_match distpar_match nodist_match jw_fname_orig-jw_name similar_0-similar_15) 
tab patient _merge
...
save "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_analysis_variables.dta", replace

* Create patient specific variables
use censusyr RecID patient patient_id using "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_analysis_variables.dta" if patient == 1, clear
drop patient
merge 1:m censusyr RecID using "$PROJ_PATH/processed/intermediate/crosswalks/pre_exist_hosp_icem_crosswalk.dta", assert(2 3) keep(3) nogen keepusing(regid)
...
save "$PROJ_PATH/processed/temp/pre_exist_hosp_analysis_variables.dta", replace

use "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_analysis_variables.dta", clear
merge 1:1 patient_id using "$PROJ_PATH/processed/temp/pre_exist_hosp_analysis_variables.dta", assert(1 3)
tab patient _merge
drop _merge
...
sort censusyr hhid pid
save "$PROJ_PATH/processed/intermediate/final_build/pre_existing_analysis_setup.dta", replace

rm "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_analysis_variables.dta"
rm "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_matched_ids.dta"
rm "$PROJ_PATH/processed/temp/pre_exist_hosp_icem_analysis_input.dta"
rm "$PROJ_PATH/processed/temp/pre_exist_hosp_analysis_variables.dta"
